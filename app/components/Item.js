...
export default class extends Component {
  ...
  render() {
    const {title, cover_urls, price, ori_price, id, status, fare, isCard } = this.props;
    let moreDesc = null;
    ...
    return (
      <li className="item">
        <Link to={`/detail/${id}`}>
          <div className="left">
            {!isCard && this.priceLogo()}
            {!isCard && Number(fare) == 0 && <img className="mailing-fee" src="http://wanliu-piano.b0.upaiyun.com/uploads/shop/poster/102/038bffe812178917d1768b9b3d6481ce.png"/>}
